...
def build_hrcnn_model(opts, gaze_weights, vocab_size=0, char_vocabsize=0, maxnum=50, maxlen=50, maxcharlen=20, embedd_dim=50, embedding_weights=None, verbose=False, init_mean_value=None, gaze_considered=5):
    ...
    if opts.use_char:
        ...
        zcnn = TimeDistributed(Conv1D(opts.nbfilters, opts.filter1_len, padding='same'), name='zcnn')(resh_W)
        resh_zcnn = Reshape((N * L, opts.nbfilters), name='resh_zcnn')(zcnn)
    ...
    if opts.l2_value:
        ...
        y_aux = []
        for i in range(gaze_considered):
            y_aux.append(Dense(units=1, activation='sigmoid', name='output_aux_' + str(i) )(resh_zcnn))

    if opts.use_char:
        model = Model(inputs=[word_input, char_input], outputs=y)
    else:
        model = Model(inputs=word_input, outputs=[y] + y_aux)
    ...
    for i in range(gaze_considered):
        loss_dict['output_aux_' + str(i) ] = 'mse'
        loss_weights['output_aux_' + str(i) ] = gaze_weights[i]
    ...
    start_time = time.time()
    model.compile(loss=loss_dict, loss_weights=loss_weights, optimizer='rmsprop')
    total_time = time.time() - start_time
    logger.info("Model compiled in %.4f s" % total_time)
...
def build_shrcnn_model(opts, gaze_weights, vocab_size=0, char_vocabsize=0, maxnum=50, maxlen=50, maxcnum=50, maxclen=50, maxcharlen=20, embedd_dim=50, embedding_weights=None, verbose=False, init_mean_value=None, gaze_considered=5):
    # LSTM stacked over CNN based on sentence level
    N = maxnum
    L = maxlen
    ...
    model = Model(inputs=[word_input, context_input], outputs=[y] + y_aux)
