...
def prepare_sentence_context_data(datapaths, embedding_path=None, embedding='word2vec', embedd_dim=100, prompt_id=1, vocab_size=0, tokenize_text=True, \
    ...
    (train_x, train_y, train_prompts, train_context, text_train, train_gaze), (dev_x, dev_y, dev_prompts, dev_context, text_dev, dev_gaze), (test_x, test_y, test_prompts, test_context, text_test, test_gaze), vocab, overal_maxlen, overal_maxnum, context_len, context_num = \
    ...
    X_train, y_train, mask_train, y_train_gaze = utils.padding_sentence_sequences_my(train_x, train_y, train_gaze, overal_maxnum, overal_maxlen, post_padding=True)
    X_dev, y_dev, mask_dev, y_dev_gaze = utils.padding_sentence_sequences_my(dev_x, dev_y, dev_gaze, overal_maxnum, overal_maxlen, post_padding=True)
    X_test, y_test, mask_test, y_test_gaze = utils.padding_sentence_sequences_my(test_x, test_y, test_gaze, overal_maxnum, overal_maxlen, post_padding=True)
    ...
    logger.info('  train Y Gaze shape: ' + str(y_train_gaze.shape))

    logger.info('  dev X shape:   ' + str(X_dev.shape))
    logger.info('  dev Y Gaze shape: ' + str(y_dev_gaze.shape))

    logger.info('  test X shape:  ' + str(X_test.shape))
    logger.info('  test Y Gaze shape: ' + str(y_test_gaze.shape))
    ...
    return (X_train, Y_train, mask_train, train_context, text_train, y_train_gaze), (X_dev, Y_dev, mask_dev, dev_context, text_dev, y_dev_gaze), (X_test, Y_test, mask_test, test_context, text_test, y_test_gaze), \
...
def prepare_sentence_data(datapaths, embedding_path=None, embedding='word2vec', embedd_dim=100, prompt_id=1, vocab_size=0, tokenize_text=True, \
    ...
    assert len(datapaths) == 3, "data paths should include train, dev and test path"
    (train_x, train_y, train_prompts, train_gaze), (dev_x, dev_y, dev_prompts, dev_gaze), (test_x, test_y, test_prompts, test_gaze), vocab, overal_maxlen, overal_maxnum = \
    ...
    X_train, y_train, mask_train, y_train_gaze = utils.padding_sentence_sequences(train_x, train_y, train_gaze, overal_maxnum, overal_maxlen, post_padding=True)
    X_dev, y_dev, mask_dev, y_dev_gaze = utils.padding_sentence_sequences(dev_x, dev_y, dev_gaze, overal_maxnum, overal_maxlen, post_padding=True)
    X_test, y_test, mask_test, y_test_gaze = utils.padding_sentence_sequences(test_x, test_y, test_gaze, overal_maxnum, overal_maxlen, post_padding=True)
    ...
    return (X_train, Y_train, mask_train, y_train_gaze), (X_dev, Y_dev, mask_dev, y_dev_gaze), (X_test, Y_test, mask_test, y_test_gaze), \
