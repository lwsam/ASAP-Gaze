...
def main():
    ...
    parser.add_argument('--train', type=str, help='train file', default='data/fold_0/train_gaze.tsv')  # "data/word-level/*.trpreprocess_asap.pyain"
    parser.add_argument('--dev', type=str, help='dev file', default='data/fold_0/dev_gaze.tsv')
    parser.add_argument('--test', type=str, help='test file', default='data/fold_0/test_gaze.tsv')
    ...
    parser.add_argument('--train_from', type=str, help='Load this saved model and continue training')
    ...
    (X_train, Y_train, mask_train, train_context, text_train, y_train_gaze), (X_dev, Y_dev, mask_dev, dev_context, text_dev, y_dev_gaze), (X_test, Y_test, mask_test, test_context, text_test, y_test_gaze), \
    ...
    model = build_shrcnn_model(args, gaze_weights, vocab_size, char_vocabsize + 1, max_sentnum, max_sentlen, context_num, context_len, maxcharlen, embedd_dim,
                              embed_table, True, init_mean_value, len(gaze_considered))


    evl = Evaluator(args.prompt_id, False, checkpoint_dir, modelname, X_train, X_dev, X_test, C_train, C_dev,
                    C_test, Y_train, Y_dev, Y_test, train_context, dev_context, test_context, y_gaze_train, y_gaze_dev, y_gaze_test)

    # Initial evaluation
    logger.info("Initial evaluation: ")
    if args.train_from:
        model.load_weights(args.train_from)
    ...
    for ii in range(args.num_epochs):
        ...
        model.fit([X_train, train_context], [Y_train, y_gaze_train[0], y_gaze_train[1], y_gaze_train[2], y_gaze_train[3], y_gaze_train[4]], batch_size=args.batch_size, epochs=1, verbose=0, shuffle=True)
