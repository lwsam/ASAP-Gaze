...
class Evaluator():
    ...
    def __init__(self, prompt_id, use_char, out_dir, modelname, train_x, dev_x, test_x, train_c, dev_c, test_c, train_y, dev_y, test_y, y_gaze_train, y_gaze_dev, y_gaze_test, char_only=False):
    ...
    def evaluate(self, model, epoch, batch_size=32, print_info=False):
        if self.use_char:
            ...
            train_pred = model.predict(self.train_x, batch_size=batch_size)[0].squeeze()
            dev_pred = model.predict(self.dev_x, batch_size=batch_size)[0].squeeze()
            test_pred = model.predict(self.test_x, batch_size=batch_size)
